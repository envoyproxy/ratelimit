...
func testNewClientImpl(t *testing.T, pipelineWindow time.Duration, pipelineLimit int) func(t *testing.T) {
	return func(t *testing.T) {
		...
		mkRedisClient := func(auth, addr string) redis.Client {
			return redis.NewClientImpl(context.Background(), statsStore, false, auth, "tcp", "single", addr, 1, pipelineWindow, pipelineLimit, nil, false, nil, 10*time.Second, "", "", time.Second, 30*time.Second, 100*time.Millisecond, 0)
		}
...
func TestDoCmd(t *testing.T) {
	...
	mkRedisClient := func(addr string) redis.Client {
		return redis.NewClientImpl(context.Background(), statsStore, false, "", "tcp", "single", addr, 1, 0, 0, nil, false, nil, 10*time.Second, "", "", time.Second, 30*time.Second, 0, 0)
	}
...
func testPipeDo(t *testing.T, pipelineWindow time.Duration, pipelineLimit int) func(t *testing.T) {
	return func(t *testing.T) {
		...
		mkRedisClient := func(addr string) redis.Client {
			return redis.NewClientImpl(context.Background(), statsStore, false, "", "tcp", "single", addr, 1, pipelineWindow, pipelineLimit, nil, false, nil, 10*time.Second, "", "", time.Second, 30*time.Second, 0, 0)
		}
...
func TestPoolOnEmptyBehavior(t *testing.T) {
	...
	mkRedisClientWithBehavior := func(addr, behavior string) redis.Client {
		return redis.NewClientImpl(context.Background(), statsStore, false, "", "tcp", "single", addr, 1, 0, 0, nil, false, nil, 10*time.Second, behavior, "", time.Second, 30*time.Second, 0, 0)
	}
...
func TestNewClientImplSentinel(t *testing.T) {
	...
	mkSentinelClient := func(auth, sentinelAuth, url string, useTls bool, timeout time.Duration) redis.Client {
		...
		// Use a short maxElapsedTime so failing connection tests don't hang in retry loops.
		return redis.NewClientImpl(context.Background(), statsStore, useTls, auth, "tcp", "sentinel", url, 1, 0, 0, nil, false, nil, timeout, "", sentinelAuth, time.Second, 30*time.Second, 100*time.Millisecond, 0)
	}
