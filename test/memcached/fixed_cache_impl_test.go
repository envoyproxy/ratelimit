...
func TestMemcached(t *testing.T) {
	...
	cache := memcached.NewFixedRateLimitCacheImpl(client, timeSource, nil, 0, nil, 0.8, "")
	...
	client.EXPECT().GetMulti([]string{"domain_key_value_1234"}).Return(
		getMultiResult(map[string]int{"domain_key_value_1234": 0}), nil,
	)
	client.EXPECT().Increment("domain_key_value_1234", uint64(1))
...
func TestOverLimitWithLocalCache(t *testing.T) {
	...
	cache := memcached.NewFixedRateLimitCacheImpl(client, timeSource, nil, 0, localCache, 0.8, "")
...
func TestMemcacheWithJitter(t *testing.T) {
	...
	statsStore := stats.NewStore(stats.NewNullSink(), false)
	cache := memcached.NewFixedRateLimitCacheImpl(client, timeSource, rand.New(jitterSource), 3600, nil, 0.8, "")
