...
func TestService(test *testing.T) {
	...
	t.cache.EXPECT().DoLimit(nil, request, []*config.RateLimit{nil}, false, ipFilter, uidFilter).Return(
	...
	t.cache.EXPECT().DoLimit(nil, request, limits, false, ipFilter, uidFilter).Return(
